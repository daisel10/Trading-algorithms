...
public class OhlcvRepository {
    ...
    /**
     * Maps a Row to OhlcvCandle object
     */
    private OhlcvCandle mapToOhlcvCandle(Row row, RowMetadata metadata) {
        OhlcvCandle candle = new OhlcvCandle();
        candle.setBucket(row.get("bucket", Instant.class));
        candle.setSymbol(row.get("symbol", String.class));
        candle.setExchange(row.get("exchange", String.class));
        candle.setOpen(row.get("open", Double.class));
        candle.setHigh(row.get("high", Double.class));
        candle.setLow(row.get("low", Double.class));
        candle.setClose(row.get("close", Double.class));
        candle.setVolume(row.get("volume", Double.class));
        return candle;
    }
}
